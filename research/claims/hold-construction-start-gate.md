...
- Forced-result behavior inside the shared gate is covered by
  `claim.judgement.forced-result-mode`; its external activation identity and
  exact timing parameter remain open.
- Record field `+0x34` is consumed with lane/width during loading, but its
  gameplay meaning is not yet assigned.
...
- Reconstruction code: shared TAP input/classification and lifecycle helpers
  already model the reused gate components.
...
start-phase guard was checked in both candidate and update functions.
